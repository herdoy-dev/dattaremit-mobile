...
import { SNAP_THEME_PICKER, SNAP_LOGOUT_MODAL } from "@/constants/limits";
    ...
    <SafeAreaView className="flex-1 bg-light-bg dark:bg-dark-bg">
      ...
      >
        ...
        <View className="px-6 pb-2 pt-4">
          <Text className="text-2xl font-bold text-light-text dark:text-dark-text">Account</Text>
        </View>
          ...
          <View className="rounded-2xl bg-light-surface px-4 dark:bg-dark-surface">
            <SettingItem
              ...
              value={
                isLoading
                  ? "..."
                  : `${user?.firstName ?? ""} ${user?.lastName ?? ""}`.trim() || "Edit profile"
              }
            ...
            <SettingItem
              ...
              label="Address"
              value={
                isLoading ? "..." : address ? `${address.city}, ${address.country}` : "No address"
              }
            ...
            <SettingItem
              ...
              onPress={
                accountStatus !== "ACTIVE" ? () => router.push("/(onboarding)/kyc") : undefined
              }
            />
          </View>
        </Animated.View>
        ...
        {/* Referral */}
        <Animated.View entering={FadeInDown.delay(250).duration(500)} className="mx-6 mt-6">
          <View className="rounded-2xl bg-light-surface px-4 dark:bg-dark-surface">
            <SettingItem
        ...
        <Animated.View entering={FadeInDown.delay(300).duration(500)} className="mx-6 mt-6">
          ...
          <View className="rounded-2xl bg-light-surface px-4 dark:bg-dark-surface">
            <SettingItem
              icon={
                isDarkMode ? <Moon size={18} color={primary} /> : <Sun size={18} color={primary} />
              }
              label="Dark Mode"
              rightElement={
                <Switch
        ...
        {/* Support */}
        <Animated.View entering={FadeInDown.delay(400).duration(500)} className="mx-6 mt-6">
          <Text className="mb-2 text-xs font-semibold uppercase tracking-wider text-light-text-muted dark:text-dark-text-muted">
            Support
          </Text>
          <View className="rounded-2xl bg-light-surface px-4 dark:bg-dark-surface">
            <SettingItem
              ...
              label="Help Center"
              onPress={() => WebBrowser.openBrowserAsync("https://dattaremit.com/help")}
            />
            <View className="h-px bg-light-border dark:bg-dark-border" />
            <SettingItem
              icon={<FileText size={18} color={primary} />}
              label="Terms & Privacy"
              onPress={() => WebBrowser.openBrowserAsync("https://dattaremit.com/legal")}
            />
          </View>
        </Animated.View>

        {/* Logout */}
        <Animated.View entering={FadeInDown.delay(500).duration(500)} className="mx-6 mt-6">
          <View className="rounded-2xl bg-light-surface px-4 dark:bg-dark-surface">
            <SettingItem
      ...
      <CustomModal
        ...
        snapPoint={SNAP_LOGOUT_MODAL}
      >
        <LogoutModal onLogout={handleLogout} onCancel={() => setShowLogoutModal(false)} />
      </CustomModal>
    </SafeAreaView>
  );
}
